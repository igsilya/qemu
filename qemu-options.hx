...
#endif
#ifdef CONFIG_AF_XDP
    "-netdev af-xdp,id=str,ifname=name[,mode=native|skb][,force-zc=on|off]\n"
    "         [,inhibit=on|off][,queues=n][,start-queue=M]\n"
    "                attach to the existing network interface 'name' with AF_XDP socket\n"
    "                use 'mode=MODE' to specify an XDP program attach mode\n"
    "                use 'force-zc=on|off' to fail if zero-copy is not available (default: off)\n"
...
``-netdev af-xdp,id=str,ifname=name[,mode=native|skb][,force-zc=on|off][,inhibit=on|off][,queues=n][,start-queue=m]``
    ...
    externally.  Number of queues 'n' should generally match the number
    or queues in the interface, defaults to 1.  'start-queue' option
    can be specified if a particular range of queues [m, m + n] should
    be in use.
